...
過去例に残るゲーム固有 Unity 層の表現は、この Unity 検証アプリ層（`kitu-integration-runner/unity-app`）に読み替えてください。
...
- Kitu ランタイム crate 群と、リポジトリ内 Unity 検証アプリ（`kitu-integration-runner/unity-app`）の境界が妥当かどうか。
...
- 配置: `kitu-integration-runner/unity-app`
...
- 検証アプリ側: `kitu-integration-runner/unity-app` の初期化 glue
...
**Unity View 層（unity-app view layer）** が処理：
...
- Unity: `com.kitu.runtime`（イベントバス）
- Unity: `unity-app`（`CameraFollow` コンポーネント）
...
- Unity: `com.kitu.runtime`（イベントバス）
- Unity: `unity-app`（`EnemySpawnerView`）
