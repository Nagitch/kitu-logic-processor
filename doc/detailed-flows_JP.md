...
本リポジトリでは、`stella-rpg` のような外部ゲーム実装リポジトリ前提を採用しません。
CI/CD と回帰確認のため、Unity 側の検証アプリは次の配置を前提とします。

- `kitu-integration-runner/unity-demo-game/`

過去例に残るゲーム固有 Unity 層の表現は、この Unity demo-game 検証アプリ層（`kitu-integration-runner/unity-demo-game`）に読み替えてください。
...
- Kitu ランタイム crate 群と、リポジトリ内 Unity demo-game 検証アプリ（`kitu-integration-runner/unity-demo-game`）の境界が妥当かどうか。
...
**Unity demo-game 検証アプリ（リポジトリ内）**

- 配置: `kitu-integration-runner/unity-demo-game`
- Unity 側: `com.kitu.runtime`（共通ブリッジ）
- Unity 側: CI/統合テストで使う検証用 View / アプリ層スクリプト
...
1. JSON → `UnityAppConfig` にデコード
2. ランタイムアプリ文脈（runtime + adapter + binding）を構築
3. 生成したアプリ文脈をグローバルに保持
...
pub extern "C" fn kitu_initialize(config_json: *const c_char) -> KituHandle {
    ...
    let app_config: UnityAppConfig = serde_json::from_str(&config_str).unwrap();
    let app = build_app(app_config).unwrap();
    store_in_global_handle(app)
}
```
...
- `kitu-unity-ffi`
- `kitu-runtime`, `kitu-data-*`, `kitu-scripting-rhai`, `kitu-tsq1`


### ランタイムアプリ bootstrap（初期化）
...
- 検証アプリ側: `kitu-integration-runner/unity-demo-game` の初期化 glue
...
**Unity View 層（unity-demo-game view layer）** が処理：
...
- Unity: `unity-demo-game`（`PlayerView`）
...
- Unity: `com.kitu.runtime`（イベントバス）
- Unity: `unity-demo-game`（`CameraFollow` コンポーネント）
...
- Unity: `com.kitu.runtime`（イベントバス）
- Unity: `unity-demo-game`（`EnemySpawnerView`）
...
- Unity: `LevelUpView`（`unity-demo-game`）
...
```rust
fn watch_tmd_changes(config: &AppConfig) {
    // デバッグビルドのみ有効
}
```
