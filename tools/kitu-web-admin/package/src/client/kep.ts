import type { ClientOscMessage, JsonOscArg } from './types.js'
...
class Writer {
  ...
  string(value: string) {
    ...
    if (data.length < 32) this.u8(0xa0 | data.length)
...
export function encodeOscPacket(message: ClientOscMessage) {
  ...
  for (const arg of message.args) {
    ...
    if (arg.type === 'int') {
      ...
      writer.raw(new Uint8Array(data))
    } else if (arg.type === 'str') writer.oscString(arg.value)
    if (writer.bytes.length < offset) throw new Error('OSC encoding failed')
  }
  return writer.finish()
}

function tag(arg: JsonOscArg) {
  if (arg.type === 'int') return 'i'
  if (arg.type === 'int64') return 'h'
  if (arg.type === 'float') return 'f'
  if (arg.type === 'str') return 's'
  return arg.value ? 'T' : 'F'
}

export function encodeKepStreamFrame(payload: Uint8Array, route: string) {
  const writer = new Writer()
  writer.u8(0x83)
  writer.string('t')
  writer.string('osc')
...
class Reader {
  ...
  byte() {
    if (this.offset >= this.bytes.length) throw new Error('incomplete KEP envelope')
    return this.bytes[this.offset++]
  }
  take(length: number) {
    const start = this.offset
    this.offset += length
    if (this.offset > this.bytes.length) throw new Error('incomplete KEP value')
  ...
  mapSize() {
    ...
    if ((marker & 0xf0) === 0x80) return marker & 15
    if (marker === 0xde) return this.u16()
        ...
        : marker === 0xd9
          ...
          : marker === 0xda
            ...
            : marker === 0xdb
              ? this.u32()
              : -1
    if (length < 0) throw new Error('unsupported KEP string')
  ...
  binary() {
    ...
    if (length < 0) throw new Error('unsupported KEP binary')
...
export function decodeKepJsonFrames(bytes: Uint8Array) {
  const events: string[] = []
  let offset = 0
  while (offset < bytes.length) {
    if (bytes.length - offset < 4) throw new Error('incomplete KEP frame')
    ...
    for (let index = 0, size = reader.mapSize(); index < size; index++) {
      ...
      if (key === 't') type = reader.string()
      else if (key === 'p') payload = reader.binary()
      else reader.skip()
    }
    if (type !== 'json' || !payload) throw new Error(`Unsupported KEP response payload: ${type || 'missing'}`)
