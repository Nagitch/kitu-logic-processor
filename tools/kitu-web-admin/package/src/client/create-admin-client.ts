...
const emptyWorld = (): WorldSnapshot => ({ tick: 0, objects: [] })

export function createAdminClient(input: AdminClientOptions) {
  const options = Object.freeze({ ...input })
  const fetcher = options.fetch ?? globalThis.fetch?.bind(globalThis)
  const connectionState = writable<ConnectionState>('idle')
  const worldSnapshot = writable<WorldSnapshot>(emptyWorld())
  const debugLogs = writable<DebugLogEntry[]>([])
  const lastError = writable<string | null>(null)
  const appActionCatalog = writable<AppActionCatalog>({ actions: [] })
  const webTransportState = writable<WebTransportState>(options.webTransportUrl ? 'closed' : 'disabled')
  const webTransportDetail = writable<string | null>(options.webTransportUrl ?? null)
  const lastOscSendStatus = writable<OscSendStatus>({ path: 'none', phase: 'idle', detail: null })
  const objectCount = derived(worldSnapshot, snapshot => snapshot.objects.length)
  let socket: WebSocketLike | null = null
  let reconnectTimer: ReturnType<typeof setTimeout> | null = null
  let transport: import('./types.js').WebTransportSessionLike | null = null
  ...
  function createSocket(url: string) {
    if (options.createWebSocket) return options.createWebSocket(url)
    return new WebSocket(url) as unknown as WebSocketLike
  }

  function start() {
    if (options.isBrowser === false || (options.isBrowser === undefined && typeof window === 'undefined')) return
    if (!stopped) return
    stopped = false
    connect()
  }
  ...
  function connect() {
    ...
    next.addEventListener('open', () => {
      if (!isCurrent(epoch) || socket !== next) return
    ...
    next.addEventListener('message', event => {
      if (!isCurrent(epoch) || socket !== next) return
      try {
        applyServerEvent(JSON.parse(String((event as MessageEvent).data)) as ServerEvent)
      } catch (error) {
    ...
    next.addEventListener('close', () => {
      ...
      if (!stopped) reconnectTimer = setTimeout(connect, options.reconnectDelayMs ?? 1500)
    })
  ...
  function resetTransport(state: WebTransportState, detail: string | null) {
  ...
  function sendOsc(payload: ClientOscMessage) {
    if (transportReady && transport) {
      ...
      void sendOverWebTransport(session, payload, epoch)
        ...
        .catch((error: unknown) => {
          if (!isCurrent(epoch) || transport !== session) return
  ...
  function sendOverWebSocket(
    ...
    path: 'websocket' | 'websocket-fallback',
    detail: string,
    expectedSocket: WebSocketLike | null = socket
  ) {
  ...
  async function sendOverWebTransport(
    session: import('./types.js').WebTransportSessionLike,
    payload: ClientOscMessage,
    epoch: number
  ) {
    let stream
    ...
    if (!isCurrent(epoch) || transport !== session) return true
    ...
    try {
      ...
      if (!isCurrent(epoch) || transport !== session) return true
      for (const json of decodeKepJsonFrames(bytes)) applyServerEvent(JSON.parse(json) as ServerEvent)
  ...
  function connectWebTransport() {
    if (!options.webTransportUrl || transport) return
    try {
      const hash = parseCertificate(options.webTransportCertificateSha256)
      if (options.webTransportCertificateSha256 && !hash) return
  ...
  function nativeWebTransport(url: string, hash?: Uint8Array) {
    const Constructor = (
      window as unknown as {
        WebTransport?: new (url: string, options?: object) => import('./types.js').WebTransportSessionLike
      }
    ).WebTransport
    if (!Constructor) throw new Error('window.WebTransport is unavailable')
    return hash
      ? new Constructor(url, { serverCertificateHashes: [{ algorithm: 'sha-256', value: hash }] })
      : new Constructor(url)
  }

  function parseCertificate(value?: string) {
  ...
  function webTransportReason() {
    if (!options.webTransportUrl) return 'WebTransport disabled'
  ...
  async function lifecycleRequest(path: string, init: RequestInit | undefined, epoch: number) {
  ...
  async function loadAppActions() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`App action catalog request failed: ${response.status}`)
  ...
  async function runAppAction(actionId: string, inputs: Record<string, ActionValue>) {
    const epoch = lifecycleEpoch
    lastError.set(null)
    lastOscSendStatus.set({ path: 'http', phase: 'pending', detail: actionId })
    try {
      const response = await lifecycleRequest(
        `/app-actions/${encodeURIComponent(actionId)}/run`,
        {
          method: 'POST',
  ...
  const spawnObject = (kind: string, x: number, y: number, z: number) =>
    runAppAction('spawn-object', {
      kind: { type: 'string', value: kind },
      x: { type: 'float', value: x },
      y: { type: 'float', value: y },
      z: { type: 'float', value: z }
    })
  const moveObject = (id: string, x: number, y: number, z: number) =>
    runAppAction('move-object', {
